// src/app/client/prescriptions/page.tsx
...
export default function ClientPrescriptionsPage() {
  ...
  const filteredPrescriptions = prescriptions.filter((prescription) =>
    ...
    prescription.veterinarian?.name?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="space-y-6 max-w-7xl mx-auto w-full font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-0.5 font-medium">
            View all your pet prescriptions
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-2xl font-black text-red-600">
            {prescriptions.filter(p => !p.isActive || isExpired(p.validUntil)).length}
          </p>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search by pet, diagnosis, or doctor..."
            value={searchTerm}
      ...
      {filteredPrescriptions.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center">
          ...
          <p className="text-slate-400 text-sm mt-1">
            {searchTerm ? 'Try adjusting your search' : 'No prescriptions yet'}
          </p>
        </div>
      ) : (
