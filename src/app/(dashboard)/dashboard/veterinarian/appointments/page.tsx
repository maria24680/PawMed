...
export default function VeterinarianAppointmentsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (user.role !== "veterinarian") return router.replace(`/dashboard/${user.role}`);
  ...
  const loadAppointments = useCallback(async (isRefresh = false) => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API_URL}/veterinarian/appointments`);
                  ...
                  <div key={a._id} className="flex flex-wrap items-center justify-between gap-3 py-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {a.client?.name ?? "Unknown client"} {a.client?.phone ? `· ${a.client.phone}` : ""}
                      </p>
